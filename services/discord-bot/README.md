...
보여줍니다. 실제 리포트 생성·발신과 신규 주문 알림은 core-api가 이 봇이 만든
**웹훅 URL**로 직접 담당합니다 — 봇은 채널·웹훅을 세팅하는 역할까지만.

## 실행 방식 두 가지

같은 기능(`/실행`, `/업데이트`, `/수익`, `/조회수`, `/일일리포트`, `/재고`,
`/연동상태`)을 서로 다른 방식으로 구현한 두 파일이 있습니다 — 하나를 골라서 씁니다.
...
| `/업데이트` | 서버 관리 | 플랜 변경이나 새 채널 종류 추가로 아직 없는 채널만 추가 생성 — 기존 채널·웹훅은 그대로 둠 (`/실행`과 달리 아무것도 지우지 않음) |
